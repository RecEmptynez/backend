...
def search_recipe(search:Search,db:Session):
    ...
    return SearchResult(recipe_names={recipe[0]: {"owned":recipe[6] if search.ingredient_names else 0 ,"total":get_num_ingredients(recipe[1],db),"url":recipe[2], "picture_url":recipe[3], "difficulty":recipe[4], "rating":recipe[5]} for recipe in result})